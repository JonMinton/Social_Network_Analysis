...
#Extract CPEP values 
cpep<-matrix(NA,10057,10057)
for(i in 1:10056)
{
 for(j in (i+1):10057)
 {
  cpep[i,j]<-lm(data[,(i+1)]~data[,(j+1)])$coef[2]
  cpep[j,i]<-lm(data[,(j+1)]~data[,(i+1)])$coef[2]
 }
}
diag(cpep)<-rep(1, 10057)
...
cpepgraph<-matrix(NA,10057,10057)
for(i in 1:10056)
{
 for(j in (i+1):10057)
 {
  ifelse(cpep[i,j]<0, temp1<-0, temp1<-cpep[i,j])
  ifelse(cpep[j,i]<0, temp2<-0, temp2<-cpep[j,i])
  if(temp1>1) temp1<-1/temp1
  if(temp2>1) temp2<-1/temp2
  cpepgraph[i,j]<-cpepgraph[j,i]<-max(temp1,temp2)
 }
# print(i)
}
